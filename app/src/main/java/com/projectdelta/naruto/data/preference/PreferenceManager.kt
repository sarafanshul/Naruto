...
class PreferenceManager(
	...
	fun filterFemale() =
		preferences.getInt(Keys.FILTER_FEMALE, Item.TriStateGroup.State.IGNORE.value)

	fun setFilterFemale(newValue: Int) =
		preferences.edit().putInt(Keys.FILTER_FEMALE, newValue).apply()
	...
	fun setFilterAlive(newValue: Int) =
		preferences.edit().putInt(Keys.FILTER_ALIVE, newValue).apply()
	...
	fun setSortAlphabetically(newValue: Int) =
		preferences.edit().putInt(Keys.SORT_ALPHA, newValue).apply()
	...
	fun setRangeEpisodeStart(newValue: Int) =
		preferences.edit().putInt(Keys.RANGE_EP_ST, newValue).apply()

	fun rangeEpisodeEnd() = preferences.getInt(Keys.RANGE_EP_ED, MAX_EPISODE_NUMBER)
	fun setRangeEpisodeEnd(newValue: Int) =
		preferences.edit().putInt(Keys.RANGE_EP_ED, newValue).apply()

	fun filterCannon() = preferences.getBoolean(Keys.FILTER_CANNON, false)
	fun setFilterCannon(newValue: Boolean) =
		preferences.edit().putBoolean(Keys.FILTER_CANNON, newValue).apply()

	fun sortAirDate() = preferences.getInt(Keys.SORT_AIR_DATE, Item.MultiSort.SORT_NONE)
	fun setSortAirDate(newValue: Int) =
		preferences.edit().putInt(Keys.SORT_AIR_DATE, newValue).apply()

	fun getAppTheme() = preferences.getInt(Keys.APP_THEME, R.style.Theme_Naruto)
	fun setAppTheme(@IntegerRes newValue: Int) =
		preferences.edit().putInt(Keys.APP_THEME, newValue).apply()
